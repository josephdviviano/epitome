...
# parse arguments
epiSubject=`opts_GetOpt1 "--subject" $@`  # "$2"
ID=`opts_GetOpt1 "--epitome-id" $@`  # "$2"
Experiment=`opts_GetOpt1 "--experiment" $@`  # "$2"
Mode=`opts_GetOpt1 "--mode" $@`  # "$2"
Session=`opts_GetOpt1 "--session" $@`  # "$2"
Run=`opts_GetOpt1 "--run" $@`  # "$2"
...
log_Msg "epiSubject: ${epiSubject}"
log_Msg "epitome ID: ${ID}"
log_Msg "Experiment: ${Experiment}"
log_Msg "Mode: ${Mode}"
...
# Setup PATHS
Subject="${Experiment}_${epiSubject}_${Session}"
PipelineScripts=${HCPPIPEDIR_fMRISurf}
HCPFolder=${HCP_DATA}
EPITOME_DATA=${EPITOME_DATA}
...
LowResMesh="32"
NameOffMRI="${Mode}_${Run}"
NativeFolder="Native"
RegName="FS"
ResultsFolder="${AtlasSpaceFolder}/Results/${Mode}/"
ROIFolder="$AtlasSpaceFolder"/ROIs
StandardSpacefMRI="${EPITOME_DATA}/${Experiment}/${epiSubject}/${Mode}/${Session}/func_MNI-nonlin.${ID}.${Run}"
...
cp ${StandardSpacefMRI}.nii.gz ${ResultsFolder}/${NameOffMRI}.nii.gz
...
# if [ ! -f ${ResultsFolder}/${NameOffMRI}_Atlas_s${SmoothingFWHM}_dil.dtseries.nii ]; then
# wb_command -cifti-dilate  ${ResultsFolder}/${NameOffMRI}_Atlas_s${SmoothingFWHM}.dtseries.nii \
#       COLUMN 10 10 \
#       ${ResultsFolder}/${NameOffMRI}_Atlas_s${SmoothingFWHM}_dil.dtseries.nii \
