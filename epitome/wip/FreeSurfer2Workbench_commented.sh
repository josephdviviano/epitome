...
#Add T1 images to the spec files
${CARET7DIR}/wb_command -add-to-spec-file \
  "$T1wFolder"/"$NativeFolder"/"$Subject".native.wb.spec INVALID \
  "$T1wFolder"/"$T1wImage".nii.gz

${CARET7DIR}/wb_command -add-to-spec-file \
  "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject".native.wb.spec INVALID \
  "$AtlasSpaceFolder"/"$T1wImage".nii.gz

${CARET7DIR}/wb_command -add-to-spec-file \
  "$AtlasSpaceFolder"/"$Subject"."$HighResMesh"k_fs_LR.wb.spec INVALID \
  "$AtlasSpaceFolder"/"$T1wImage".nii.gz

for LowResMesh in ${LowResMeshes} ; do
  ${CARET7DIR}/wb_command -add-to-spec-file \
  "$AtlasSpaceFolder"/fsaverage_LR"$LowResMesh"k/"$Subject"."$LowResMesh"k_fs_LR.wb.spec \
  INVALID "$AtlasSpaceFolder"/"$T1wImage".nii.gz

  ${CARET7DIR}/wb_command -add-to-spec-file \
  "$T1wFolder"/fsaverage_LR"$LowResMesh"k/"$Subject"."$LowResMesh"k_fs_LR.wb.spec \
  INVALID "$T1wFolder"/"$T1wImage".nii.gz
done
...
for Hemisphere in L R ; do
  ...
  for Surface in white pial ; do
    ...
    ## converting the surface from freesurfer add some header info to gifti, apply cras affine
    mris_convert "$FreeSurferFolder"/surf/"$hemisphere"h."$Surface" \
      "$T1wFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$Surface".native.surf.gii
    ${CARET7DIR}/wb_command -set-structure \
      "$T1wFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$Surface".native.surf.gii \
      ${Structure} -surface-type $Type$Secondary
    ${CARET7DIR}/wb_command -surface-apply-affine \
      "$T1wFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$Surface".native.surf.gii \
      "$FreeSurferFolder"/mri/c_ras.mat \
      "$T1wFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$Surface".native.surf.gii
    ## add the native surface to the spec file
    ${CARET7DIR}/wb_command -add-to-spec-file \
      "$T1wFolder"/"$NativeFolder"/"$Subject".native.wb.spec \
      $Structure "$T1wFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$Surface".native.surf.gii
    #apply linear and non-linear transforms to MNI space
    ...
    # add the MNIspace surfaces to the spec file
    ${CARET7DIR}/wb_command -add-to-spec-file \
      "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject".native.wb.spec \
      $Structure \
      "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$Surface".native.surf.gii \
    #increment i
    i=$(($i+1))
  done
