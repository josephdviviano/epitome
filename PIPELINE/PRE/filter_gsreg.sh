    ...
    for SESS in ${DIR_SESS}; do
        ...
        for RUN in ${DIR_RUNS}; do
            ...
            if [ ! -f ${SESS}/PARAMS/det.wm_local15.${NUM}.nii.gz ]; then
                3dDetrend -prefix ${SESS}/PARAMS/det.wm_local15.${NUM}.nii.gz \
                          -polort ${POLORT} \
                          ${SESS}/PARAMS/wm_local15.${NUM}.nii.gz
            fi

            if [ ! -f ${SESS}/PARAMS/det.lag.wm_local15.${NUM}.nii.gz ]; then
                3dDetrend -prefix ${SESS}/PARAMS/det.lag.wm_local15.${NUM}.nii.gz \
                          -polort ${POLORT} \
            ...
            if [ ! -f ${SESS}/func_noise.${NUM}.nii.gz ]; then
                3dTfitter -prefix ${SESS}/func_noise_betas.${NUM}.nii.gz \
            ...
            if [ ! -f ${SESS}/func_filtered.${NUM}.nii.gz ]; then
                3dTstat -prefix ${SESS}/func_tmp_mean.${NUM}.nii.gz \
                        -mean ${SESS}/func_scaled.${NUM}.nii.gz
                3dTstat -prefix ${SESS}/func_tmp_stdev.${NUM}.nii.gz \
                        -stdev ${SESS}/func_scaled.${NUM}.nii.gz
                
                # compute temporal SNR
                3dcalc -a ${SESS}/func_tmp_mean.${NUM}.nii.gz \
