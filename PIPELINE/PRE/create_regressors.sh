...
for SUB in ${SUBJECTS}; do
    ...
    for SESS in `basename ${DIR_SESS}`; do
        ...
        if [ ! -f ${DIR}/anat_wm_ero.nii.gz ]; then
            3dcalc -a ${DIR}/anat_aparc_reg.nii.gz \
                   -expr "equals(a,2)  + \
                          equals(a,7)  + \
                          equals(a,41) + \
                          equals(a,46) + \
        ...
        if [ ! -f ${DIR}/anat_EPI_mask_dia.nii.gz ]; then
            3dcalc -a ${DIR}/anat_EPI_mask.nii.gz \
                   -b a+i -c a-i -d a+j -e a-j -f a+k -g a-k \
                   -expr 'amongst(1,a,b,c,d,e,f,g)' \
        ...
        for RUN in ${DIR_RUNS}; do
            ...
            if [ ! -f ${DIR}/PARAMS/lag.wm_local15.${NUM}.nii.gz ]; then
                3dLocalstat -prefix ${DIR}/PARAMS/wm_local15.${NUM}.nii.gz \
                            -nbhd 'SPHERE(15)' \
                            -stat mean \
                            -mask ${DIR}/anat_wm_ero.nii.gz \
                            -use_nonmask ${DIR}/func_scaled.${NUM}.nii.gz

                3dTcat -prefix ${DIR}/PARAMS/lag.wm_local15.${NUM}.nii.gz \
            ...
            if [ ! -f ${DIR}/PARAMS/lag.vent.${NUM}.1D ]; then
                3dmaskave -q -mask ${DIR}/anat_vent_ero.nii.gz \
                          ${DIR}/func_scaled.${NUM}.nii.gz > \
                          ${DIR}/PARAMS/vent.${NUM}.1D
                1dcat ${DIR}/PARAMS/vent.${NUM}.1D'{0}' > \
            ...
            if [ ! -f ${DIR}/PARAMS/lag.dv.${NUM}.1D ]; then
                3dmaskave -q -mask ${DIR}/anat_dv_ero.nii.gz \
                          ${DIR}/func_scaled.${NUM}.nii.gz > \
                          ${DIR}/PARAMS/dv.${NUM}.1D
                1dcat ${DIR}/PARAMS/dv.${NUM}.1D'{0}' > \
