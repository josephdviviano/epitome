...
Times series normalization can be accomplished in one of three ways: zscores, percent signal change, and a simple demean. In all cases, the mean of each time series is set to a single number (either 0 or 100 for `pct'). This will help normalize baseline shifts across sessions, runs, and participants. Optionally, one can convert to percent signal change, which normalizes by time series mean and retains amplitude fluctuations across the image, and zscores, which normalizes by standard deviation and removes most amplitude differences across the image. Your selection here might be motivated by personal preference, or in rarer cases, analytic requirements. When in doubt, it is safe to select `demean'.
